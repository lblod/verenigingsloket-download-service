 ...
 */
async function buildHeaders(sessionId) {
 ...
 * @param {string} sessionId - The mu-session-id for resolving OAuth2 client
 * @returns {Object|null} The API response or null on error
 */
export async function fetchAssociationByVCode(vCode, sessionId) {
 ...
 */
export async function fetchAssociationsFromAPI(vCodes, sessionId) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const chunkResults = await Promise.all(
      chunk.map(vCode => fetchAssociationByVCode(vCode, sessionId))
    );
